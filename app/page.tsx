...
type Product = { product_id:string; sku:string; manufacturer:string; catalog_section_source:string; product_name_source:string; product_name_search:string; technical_specifications:string; specification_status:string; source_pdf:string; source_pdf_page:number; image_file:string; model_number?:string; short_description?:string; detailed_description?:string; };
...
const IMAGE_VERSION="20260824-royal-large";
...
export default function Home(){
 ...
 if(selectedProduct){const filename=selectedProduct.image_file.split("/").pop();const image=filename?`${ASSET_BASE}/product-images/${filename}?v=${IMAGE_VERSION}`:undefined;return <main><ProductDetail product={selectedProduct} image={image} onBack={()=>setSelectedProduct(null)}/></main>}
 return <main>
