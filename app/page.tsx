...
export default function Home(){
 ...
 useEffect(()=>{Promise.all([fetch("/data/products.json").then(r=>r.json()),readAllImages()]).then(([catalog,savedImages]:[Product[],Record<string,Blob|string>])=>{setProducts(catalog);const urls:Record<string,string>={},removed:Record<string,boolean>={};Object.entries(savedImages).forEach(([id,value])=>{if(value instanceof Blob)urls[id]=URL.createObjectURL(value);else if(value==="__removed__")removed[id]=true});setImageUrls(urls);setRemovedImages(removed);setLoading(false)})},[]);
 ...
 if(selectedProduct){const filename=selectedProduct.image_file.split("/").pop();const image=removedImages[selectedProduct.product_id]?undefined:(imageUrls[selectedProduct.product_id]||(filename?`/product-images/${filename}`:undefined));return <main><ProductDetail product={selectedProduct} image={image} onBack={()=>setSelectedProduct(null)}/></main>}
 return <main>
