...
const DB_NAME="medical-catalog-images", STORE_NAME="product-images";
...
function openImageDb():Promise<IDBDatabase>{return new Promise((resolve,reject)=>{const request=indexedDB.open(DB_NAME,1);request.onupgradeneeded=()=>{if(!request.result.objectStoreNames.contains(STORE_NAME))request.result.createObjectStore(STORE_NAME)};request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error)})}
async function readAllImages():Promise<Record<string,Blob|string>>{const db=await openImageDb();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE_NAME,"readonly"),store=tx.objectStore(STORE_NAME),keys=store.getAllKeys(),values=store.getAll();tx.oncomplete=()=>{const result:Record<string,Blob|string>={};keys.result.forEach((key,index)=>{result[String(key)]=values.result[index] as Blob|string});db.close();resolve(result)};tx.onerror=()=>reject(tx.error)})}
async function saveImage(id:string,file:Blob){const db=await openImageDb();await new Promise<void>((resolve,reject)=>{const tx=db.transaction(STORE_NAME,"readwrite");tx.objectStore(STORE_NAME).put(file,id);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error)});db.close()}
async function markImageRemoved(id:string){const db=await openImageDb();await new Promise<void>((resolve,reject)=>{const tx=db.transaction(STORE_NAME,"readwrite");tx.objectStore(STORE_NAME).put("__removed__",id);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error)});db.close()}

function ProductCard({product,image,onUpload,onRemove,onMoreInfo}:{product:Product;image?:string;onUpload:(id:string,file:File)=>void;onRemove:(id:string)=>void;onMoreInfo:(product:Product)=>void}){
 const inputRef=useRef<HTMLInputElement>(null);
 const handleFile=(event:ChangeEvent<HTMLInputElement>)=>{const file=event.target.files?.[0];if(file)onUpload(product.product_id,file);event.target.value=""};
 return <article className="product-card">
  <div className="image-box">
   ...
   <input ref={inputRef} type="file" accept="image/png,image/webp,image/jpeg" onChange={handleFile} hidden/>
   <div className="image-actions"><button type="button" className="upload-button" onClick={()=>inputRef.current?.click()}>{image?"Change image":"Upload image"}</button>{image&&<button type="button" className="remove-button" onClick={()=>onRemove(product.product_id)}>Remove</button>}</div>
  </div>
...
export default function Home(){
 const [products,setProducts]=useState<Product[]>([]),[query,setQuery]=useState(""),[company,setCompany]=useState("All companies"),[panelOpen,setPanelOpen]=useState(false),[selectedProduct,setSelectedProduct]=useState<Product|null>(null),[imageUrls,setImageUrls]=useState<Record<string,string>>({}),[removedImages,setRemovedImages]=useState<Record<string,boolean>>({}),[loading,setLoading]=useState(true);
 useEffect(()=>{Promise.all([fetch(`${ASSET_BASE}/data/products.json`).then(r=>r.json()),readAllImages()]).then(([catalog,savedImages]:[Product[],Record<string,Blob|string>])=>{setProducts(catalog);const urls:Record<string,string>={},removed:Record<string,boolean>={};Object.entries(savedImages).forEach(([id,value])=>{if(value instanceof Blob)urls[id]=URL.createObjectURL(value);else if(value==="__removed__")removed[id]=true});setImageUrls(urls);setRemovedImages(removed);setLoading(false)})},[]);
 ...
 const handleUpload=async(id:string,file:File)=>{if(!file.type.startsWith("image/"))return;await saveImage(id,file);setRemovedImages(current=>{const next={...current};delete next[id];return next});setImageUrls(current=>{if(current[id])URL.revokeObjectURL(current[id]);return{...current,[id]:URL.createObjectURL(file)}})};
 const handleRemove=async(id:string)=>{await markImageRemoved(id);setRemovedImages(current=>({...current,[id]:true}));setImageUrls(current=>{if(current[id])URL.revokeObjectURL(current[id]);const next={...current};delete next[id];return next})};
 if(selectedProduct){const filename=selectedProduct.image_file.split("/").pop();const image=removedImages[selectedProduct.product_id]?undefined:(imageUrls[selectedProduct.product_id]||(filename?`${ASSET_BASE}/product-images/${filename}`:undefined));return <main><ProductDetail product={selectedProduct} image={image} onBack={()=>setSelectedProduct(null)}/></main>}
 return <main>
